...
-- weather event watcher (attribute-driven: WeatherValues attrs + predictor transitions)
local WeatherWatch = {
    events = {},
    night = false,
    weather = nil,
...
local WeatherHUD = WeatherPredictor
    ...
    for _, loc in ipairs(spawns:GetChildren()) do
        if loc.Parent then
            ...
            if has then
                ...
                if valid then
                    if Library.Flags["rareNotify"] and rare then
...
task.spawn(function()
    ...
    while task.wait(2) do
        ...
        pcall(function()
            ...
            -- weather labels, predictor, HUD and value ESP refresh run in the local 1s predictor loop
            -- ESP
...
-- Local predictor / HUD / value ESP sync loop (1s; zero external network calls)
task.spawn(function()
    while task.wait(1) do
        ...
        pcall(function()
            ...
            -- moon/phase transition alerts (revived: the old WeatherEffects packet hooks died in the game update)
...
notify("GAG2", "9 tabs loaded - Weather HUD - Value ESP - Local Predictor", "info")
