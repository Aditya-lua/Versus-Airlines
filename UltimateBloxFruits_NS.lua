...
-- Versus Airlines wrapper that mimics Fluent API
local Tabs = {}
local Window = ui

local function wrapTab(name)
    local tab = ui:CreateSection(name)
    local function makeWrappedSection(sectionName)
        if sectionName then tab:AddSection(sectionName) end  -- wait, VA doesn't have sub-sections like Fluent
        local s = {}
        function s:AddToggle(id, opts)
            opts = opts or {}
            tab:createToggle({
                Name = opts.Title or "",
                flagName = id,
                Flag = opts.Default or false,
                Callback = opts.callback or function(v) Library.Flags[id] = v end,
            })
            local proxy = {}
            function proxy:OnChanged(cb)
                -- Store callback for later - VA calls Callback directly
                local origCb = opts.callback
                tab:createToggle({
                    Name = opts.Title or "",
                    flagName = id,
                    Flag = opts.Default or false,
                    Callback = function(v) Library.Flags[id] = v; cb(v) end,
                })
            end
            return proxy
        end
        ...
        function s:AddLabel(opts)
            opts = opts or {}
            tab:createLabel({ Name = opts.Title or "" })
        end
        function s:AddParagraph(opts)
            opts = opts or {}
            tab:createLabel({ Name = opts.Title or "" })
            local proxy = {}
            function proxy:SetDesc(text) end
            return proxy
        end
        function s:AddDropdown(id, opts)
            opts = opts or {}
            local dd = tab:createDropdown({
                Name = opts.Title or "",
                flagName = id,
                List = opts.Values or {},
                Flag = opts.Default or (opts.Values and opts.Values[1]) or "",
                multi = opts.Multi or false,
            })
            local proxy = {}
            function proxy:OnChanged(cb)
                Library.Flags[id] = Library.Flags[id] or opts.Default or ""
            end
            function proxy:Set(values) if dd and dd.updateList then dd:updateList(values) end end
            return proxy
        end
        function s:AddSlider(id, opts)
            opts = opts or {}
            tab:createSlider({
                Name = opts.Title or "",
                flagName = id,
                value = opts.Default or 0,
                minValue = opts.Min or 0,
                maxValue = opts.Max or 100,
            })
            local proxy = {}
            function proxy:OnChanged(cb) end
            return proxy
        end
        function s:AddInput(id, opts)
            opts = opts or {}
            tab:createInputBox({
                Name = opts.Title or "",
                flagName = id,
                Flag = opts.Default or "",
                Callback = opts.Callback or function() end,
            })
        end
        function s:AddSection(name)
            -- Versus Airlines doesn't have nested sections in the same way
            -- Just return self so calls chain properly
            return s
        end
        return s
    end
    return makeWrappedSection()
end
